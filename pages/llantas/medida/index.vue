  ...
  <section class="container">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <article class="col-span-9">
        <p class="text-2xl font-medium">Llantas Medida: {{ medida }}</p>
...
export default {
  ...
  async asyncData({ $http, $axios, query }) {
    const { width, profile, rin } = query
    const medida = `${width}/${profile} R${rin}`
    const llantas = await $axios.$get(
      `/llantas/?width=${width}&profile=${profile}&rin=${rin}`
    )
    console.log(llantas)
    return { llantas, medida }
  },
  data() {
    return {
      cantLlantas: [],
  ...
  mounted() {
    for (let index = 0; index < this.llantas.length; index++) {
      this.cantLlantas.push(1)
    }
  },
}
